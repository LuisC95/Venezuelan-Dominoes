-- La UI necesita el reloj del servidor.
--
-- La cuenta de segundos sin señal se comparaba con `Date.now()` del navegador.
-- Como `last_seen_at` lo pone el servidor, cualquier desfase de reloj —y en un
-- teléfono son segundos— corría la cuenta: el botón de anular la mano aparecía
-- mientras `void_hand` seguía respondiendo "el jugador de turno sigue
-- conectado". Devolviendo `now()` el cliente calcula el desfase y cuenta con la
-- hora buena.
--
-- create or replace: conserva los grants de
-- 20260903025802_lock_down_function_grants.sql.
